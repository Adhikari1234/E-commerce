...
const Categories = ({ mainData, setProductData }) => {
  ...
  const setCategories = (category) => {
    ...
    const tempArray = mainData.filter((item) => item.mealType[0] === category);
    ...
    <div className="my-4 mx-3 md:mx-8">
      ...
      <div className="w-fit flex gap-7 md:gap-16 lg:gap-28 mx-auto my-4">
        {icons.map((item) => (
          ...
          >
            <div
              className={`bg-gray-300 border rounded-full p-1 transition-all duration-300
              ${item.name === pick
                  ? "border-2 border-orange-500 bg-gray-600 shadow-md shadow-black/50"
                  : "border-gray-300 hover:border-gray-700 hover:bg-gray-700"}
              `}
            >
              <img src={item.src} alt={item.name} className="h-7 w-7 md:h-11 md:w-11" />
            </div>
